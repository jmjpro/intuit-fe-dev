...
describe('fsm', () => {
  ...
  it('should error when trying to create transitions on nonexistent states', () => {
    ...
    expect(() =>
      turnstile.addTransition(nonexistentStartState, 'coin', 'unlocked'),
    ).toThrowError("state 'state1' not found")
    expect(() =>
      turnstile.addTransition('locked', 'coin', nonexistentEndState),
    ).toThrowError("state 'state2' not found")
  })
