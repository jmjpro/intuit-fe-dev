    ...
    <>
      ...
      <main className={styles.main}>
        <div className={styles.directions}>
          <Info size={16} />
          Add states, add transitions between the states, then click transition
          buttons to run transitions, and notice how the state changes
        </div>
        ...
        <div>
          ...
          <button onClick={() => setExample('turnstile')}>
            Load turnstile FSM{' '}
            {example === 'turnstile' ? <Spinner /> : <Loader size={16} />}
          </button>
          <button onClick={() => setExample('trafficlight')}>
            Load traffic light FSM{' '}
            {example === 'trafficlight' ? <Spinner /> : <Loader size={16} />}
          </button>
        </div>
      </main>
    </>
